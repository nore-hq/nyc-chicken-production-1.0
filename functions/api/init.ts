...
CREATE TABLE IF NOT EXISTS admin_users (
  ...
  const seed = `
INSERT OR IGNORE INTO admin_users (id, username, password_hash, salt, name) 
VALUES ('admin_1', 'nycadmin@nyc.com', '20e7df12fabce1cdfa43d2069dde39c03cd564c0c0f0e4a3b07b2f425820b657', 'ab2587a7a9c924c56420f67f4e625797', 'Restaurant Manager');
  ...
  try {
    // D1 requires multiple statements to be split or executed via batch
    const statements = schema.split(';').filter(s => s.trim().length > 0).map(s => context.env.DB.prepare(s));
    statements.push(context.env.DB.prepare(seed));
